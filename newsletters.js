...
(function () {
  ...
  async function loadNewsletterEntry(entry) {
    ...
    try {
      ...
      return {
        ...
        title,
        dateIso,
        formattedDate: formatMonthYear(dateIso),
        sortDate: dateIso ? new Date(dateIso) : new Date(0),
        html,
        error: null
      };
    } catch (error) {
      ...
      return {
        ...
        formattedDate: formatMonthYear(fallback.date),
        sortDate: fallback.date ? new Date(fallback.date) : new Date(0),
        html: "",
  ...
  function inferFromFilename(fileName) {
    ...
    const monthName = new Date(date).toLocaleDateString(undefined, { month: "long" });
